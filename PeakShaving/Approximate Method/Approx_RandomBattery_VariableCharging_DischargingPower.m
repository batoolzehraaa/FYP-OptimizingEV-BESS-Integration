...
% Define initial charging/discharging power levels
charging_power = 10;  % kW (initial)
discharging_power = 20;  % kW (initial)
target_SOC_max = 90;  % Max SOC for charging
target_SOC_min = 30;  % Min SOC for discharging
...
% Load Profile for 24 hours (example load in kW)
loadProfile = [0.8, 0.9, 1.0, 1.1, 1.3, 1.5, 1.8, 2.0, 2.3, 2.4, 2.5, 2.7, ...
               2.4, 1, 2.8, 2.1, 2.2, 2.2, 2.0, 1.8, 1.7, 1.6, 1.4, 1.2, 1.0];
...
while simTime < endTime
    % Fetch the latest voltages from OpenDSS (for grid voltage, if required)
    ...
    S_total = sqrt(P1^2 + Q1^2);  % Total apparent power (kVA)
    ...
    if simTime > 12 * 3600
        ...
        if SOC_value > target_SOC_min
            discharging_power = 20;  
        end
    end
    ...
    if SOC_value < target_SOC_max
        ...
        P_BESS =- charging_power*currentLoad;  % Set dynamic charging power
        DSSText.Command = ['Storage.BESS1.KW=', num2str(P_BESS)];
        DSSText.Command = 'edit Storage.BESS1 State=CHARGING';
        P_total = -P_total;  % Reverse the total grid power to represent discharge

        % Energy consumed by the grid (charging power)
    ...
    elseif SOC_value > target_SOC_min
        % If SOC is above 10%, discharge the battery
        P_BESS = discharging_power*currentLoad;  % Set dynamic discharging power
        DSSText.Command = ['Storage.BESS1.KW=', num2str(P_BESS)];
